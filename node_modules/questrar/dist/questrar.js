(function (global, factory) {
  ...
  (factory((global.Questrar = {}),global.React,global.PropTypes,global.ReactDom));
   ...
   */
  var arrayValuesOfKeys = function arrayValuesOfKeys(obj, keys, getValue) {
    invariant_1(nonEmpty(obj), 'No object has been provided for keys extraction');
    invariant_1(nonEmpty(keys), 'There are no keys provided for values extraction');
  ...
  var css = ".errorContainer{width:100%;height:100%;display:flex;flex-flow:column nowrap;justify-content:center;align-items:center;text-align:center;font-size:20px}.requestFailurePopover .Popover-body{border:1px solid red}.requestFailurePopover .Popover-tipShape{stroke:red;stroke-width:1px}";
  styleInject(css);
  ...
  var reIsNative = RegExp('^' +
    fnToString.call(hasOwnProperty$1).replace(/[\\^$.*+?()[\]{}|]/g, '\\$&')
  ...
  var platform_1 = platform$1.isServer;
  var platform_2 = platform$1.isClient;
  var platform_3 = platform$1.window;
  var platform_4 = platform$1.document;
  ...
  var clientOnly = function clientOnly(f) {
    return platform$1.isClient ? f : noop;
  };
  ...
  El.calcSize = function (el) {
    return el === platform$1.window ? { w: el.innerWidth, h: el.innerHeight } : { w: el.offsetWidth, h: el.offsetHeight };
  };

  El.calcScrollSize = function (el) {
    return el === platform$1.window ? {
  ...
  var requestAnimationFrame = platform$1.isServer ? utils.noop : platform$1.window.requestAnimationFrame || platform$1.window.mozRequestAnimationFrame || platform$1.window.webkitRequestAnimationFrame || function (fn) {
    platform$1.window.setTimeout(fn, 20);
  };

  var cancelAnimationFrame = platform$1.isServer ? utils.noop : platform$1.window.cancelAnimationFrame || platform$1.window.mozCancelAnimationFrame || platform$1.window.webkitCancelAnimationFrame || platform$1.window.clearTimeout;

  var isIE = platform$1.isServer ? false : navigator.userAgent.match(/Trident/);
  ...
  var on = function on(el, fn) {
    ...
    if (el === platform$1.window) {
      platform$1.window.addEventListener("resize", fn);
      return;
    }
  ...
  var off = function off(el, fn) {
    if (el === platform$1.window) {
      platform$1.window.removeEventListener("resize", fn);
      return;
    }
    var detector = el[namespace];
    if (!detector) return;
   ...
   */
  var RequestError = function (_React$Component) {
    inherits$2(RequestError, _React$Component);
    ...
    function RequestError() {
      ...
      return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = RequestError.__proto__ || Object.getPrototypeOf(RequestError)).call.apply(_ref, [this].concat(args))), _this), _this.state = { open: true }, _this._closeTooltip = function () {
        var _this$props = _this.props,
            id = _this$props.id,
            actions = _this$props.actions,
            onCloseError = _this$props.onCloseError,
            request = _this$props.request;

        //handle callback

        if (_this.state.open && isFunc(onCloseError)) {
          onCloseError({ data: request, actions: actions });
        }
        //remove request if set to autoRemove
        if (_this.state.open && (request.autoRemove || request.removeOnFail)) {
          actions.remove(id);
        } else {
          actions.dirty(id);
        }
        _this.setState({ open: false });
      }, _this._children = function () {
        var _this$props2 = _this.props,
            children = _this$props2.children,
            request = _this$props2.request,
            inject = _this$props2.inject,
            actions = _this$props2.actions;

        var injection = { request: { data: request, actions: actions } };

        if (React__default.isValidElement(children)) {
          //Transform requestState to child props via inject function
          if (isFunc(inject)) {
            var params = inject(injection.request);
            var paramProps = isObj(params) ? params : { request: params };
            //$FlowFixMe
            return React__default.cloneElement(children, paramProps);
          }
          if (inject) {
            //$FlowFixMe
            return React__default.cloneElement(children, injection);
          }
        }
        return children;
        ...
        if (request.message.title) {
          title = React__default.isValidElement(request.message.title) ? request.message.title : React__default.createElement(
            'div',
            { className: 'q-tooltip-title-failed' },
            request.message.title
          );
        }

        var message = request.message.body ? request.message.body : request.message;

        return React__default.createElement(
          'div',
          { className: 'q-tooltip-content-failed', onClick: _this._closeTooltip },
          title,
          message
        );
    ...
    createClass(RequestError, [{
      ...
      value: function render() {
        var _props = this.props,
            errorTooltip = _props.errorTooltip,
            request = _props.request,
            passiveOnError = _props.passiveOnError,
            _props$className = _props.className,
            className = _props$className === undefined ? '' : _props$className;


        if (errorTooltip && request.message) {
          var classNames = 'requestFailurePopover ' + className;
        ...
        if (React__default.isValidElement(request.message)) {
  ...
  var Spinner = function Spinner(_ref) {
    ...
    var style = {};
    var _size = isNumber(size) ? size : 40;
    Object.assign(style, { width: _size, height: _size });

    var cogs = Array(11).fill(1);

    return React__default.createElement(
      'div',
      { className: 'sk-fading-circle', style: style },
      cogs.map(function (c, i) {
        var className = 'sk-circle sk-circle' + (i + 2);
        return React__default.createElement('div', { className: className, key: '' + i });
      })
    );
  };

  var css$2 = ".pendingContainer{width:100%;height:100%;display:flex;flex-flow:column nowrap;justify-content:center;align-items:center;text-align:center;font-size:20px}";
  styleInject(css$2);
   ...
   * @constructor
   */
  var RequestPending = function RequestPending(_ref) {
      var color = _ref.color;

      return React__default.createElement(
          'div',
          { className: 'pendingContainer' },
          React__default.createElement(Spinner, { size: 20, color: color })
      );
  };

  var css$3 = ".successContainer{width:100%;height:100%;display:flex;flex-flow:column nowrap;justify-content:center;align-items:center;text-align:center;font-size:20px}.requestSuccessPopover .Popover-body{border:1px solid #37b320}.requestSuccessPopover .Popover-tipShape{stroke:#37b320;stroke-width:1px}";
  styleInject(css$3);

  var RequestSuccess = function (_React$Component) {
    inherits$2(RequestSuccess, _React$Component);
    ...
    function RequestSuccess() {
      ...
      classCallCheck(this, RequestSuccess);

      for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
        args[_key] = arguments[_key];
      }

      return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = RequestSuccess.__proto__ || Object.getPrototypeOf(RequestSuccess)).call.apply(_ref, [this].concat(args))), _this), _this.state = { open: true }, _this._defaultSuccessMessage = 'Successful', _this._closeTooltip = function () {
        var _this$props = _this.props,
            id = _this$props.id,
            actions = _this$props.actions,
            onCloseSuccess = _this$props.onCloseSuccess,
            request = _this$props.request;

        //handle callback

        if (_this.state.open && isFunc(onCloseSuccess)) {
          onCloseSuccess({ data: request, actions: actions });
        }
        //remove request if set to autoRemove
        if (_this.state.open && (request.autoRemove || request.removeOnSuccess)) {
          actions.remove(id);
        } else {
          actions.dirty(id);
        }
        ...
        if (request.message.title) {
          title = React__default.isValidElement(request.message.title) ? request.message.title : React__default.createElement(
            'div',
            { className: 'q-tooltip-title-success' },
            request.message.title
          );
        }
        ...
        return React__default.createElement(
          'div',
          { className: 'q-tooltip-content-success', onClick: _this._closeTooltip },
          title,
          message
        );
      }, _temp), possibleConstructorReturn(_this, _ret);
    ...
    createClass(RequestSuccess, [{
      ...
      value: function render() {
        var _props = this.props,
            ...
            successTooltip = _props.successTooltip,
            successReplace = _props.successReplace,
            request = _props.request,
            _props$className = _props.className,
            className = _props$className === undefined ? '' : _props$className;
        ...
        if (successReplace) {
          ...
          return React__default.createElement(
            ...
            React__default.createElement(
              'div',
              null,
              nonEmpty(request.message) && !isObj(request.message) ? request.message : this._defaultSuccessMessage
            )
          );
        }

        if (successTooltip && request.message) {
          var classNames = 'requestSuccessPopover ' + className;
  ...
  var _React$createContext = React__default.createContext({}),
      RequestProviderContext = _React$createContext.Provider,
      RequestConsumerContext = _React$createContext.Consumer;
   ...
   */
  function withRequestSelector(WrappedComponent) {
    return function (_React$Component) {
      inherits$2(_class2, _React$Component);
      ...
      function _class2() {
        ...
        return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = _class2.__proto__ || Object.getPrototypeOf(_class2)).call.apply(_ref, [this].concat(args))), _this), _this._getRequest = function (state) {

          if (_this.props.id && Object.hasOwnProperty.call(state, _this.props.id)) {
            return state[_this.props.id];
          }
          return initialRequest;
        }, _this.renderComponent = function (state) {
          var data = _this._getRequest(state.data);
          var request = Object.assign({}, data, { id: _this.props.id });

          return React__default.createElement(WrappedComponent, _extends({}, _this.props, { actions: state.actions, request: request }));
        }, _temp), possibleConstructorReturn(_this, _ret);
      }
  ...
  var css$4 = ".Popover-body{background-color:#fff;border-radius:5px;font-size:14px;max-width:400px}.Popover-tipShape{stroke-dasharray:0,0,20;stroke-width:1px;stroke-linejoin:round}.Popover-tip{fill:#fff}.Popover-above .Popover-tip{margin-top:-1px}.Popover-below .Popover-tip{margin-bottom:-1px}.Popover-left .Popover-tip{margin-left:-1px}.Popover-right .Popover-tip{margin-right:-1px}.q-tooltip-content-failed,.q-tooltip-content-success{padding:5px;text-align:center;cursor:pointer}.q-tooltip-content-failed .q-tooltip-title-failed,.q-tooltip-content-failed .q-tooltip-title-success,.q-tooltip-content-success .q-tooltip-title-failed,.q-tooltip-content-success .q-tooltip-title-success{font-size:16px;padding-bottom:5px}";
  styleInject(css$4);
   ...
   * @constructor
   */
  var RequestComponent = function RequestComponent(_ref) {
    var id = _ref.id,
        ...
        failTooltip = _ref.failTooltip,
        onCloseFailure = _ref.onCloseFailure,
        onFailure = _ref.onFailure,
        passiveOnFailure = _ref.passiveOnFailure,
        renderLoading = _ref.renderLoading,
        passivePending = _ref.passivePending,
        initialPending = _ref.initialPending,
        renderInitial = _ref.renderInitial,
        onSuccess = _ref.onSuccess,
        successTooltip = _ref.successTooltip,
        successReplace = _ref.successReplace,
        onCloseSuccess = _ref.onCloseSuccess,
        color = _ref.color,
        className = _ref.className;


    invariant_1(id !== null && id !== undefined, 'No request state id is provided as a prop');
    ...
    if (request.pending && !passivePending) {
      ...
      if (renderLoading && isFunc(inject)) {
        ...
        var paramProps = isObj(params) ? params : { request: params };
        // $FlowFixMe
        return React__default.cloneElement(renderLoading, paramProps);
      }

      if (renderLoading && inject) {
        // $FlowFixMe
        return React__default.cloneElement(renderLoading, injection);
      }

      if (React__default.isValidElement(renderLoading)) {
        return renderLoading;
      }

      return React__default.createElement(RequestPending, { color: color });
    }

    //if request isPending, keep child as loading element. Dont replace child.
    if (request.pending && passivePending) {
      var singleChild = React__default.Children.count(children) === 1;
      ...
      if (singleChild && isFunc(inject)) {
        ...
        var _params = inject(injection.request);
        var _paramProps = isObj(_params) ? _params : { request: _params };
        //$FlowFixMe
        return React__default.cloneElement(children, _paramProps);
      }
    ...
    if (request.failed) {
      ...
      if (React__default.isValidElement(renderFailure)) {
        if (isFunc(inject)) {
          var _params2 = inject(injection.request);
          var _paramProps2 = isObj(_params2) ? _params2 : { request: _params2 };
          // $FlowFixMe
          return React__default.cloneElement(renderFailure, _paramProps2);
        }

        return inject ? React__default.cloneElement(renderFailure, request) : renderFailure;
      }
    ...
    if (request.success) {
      ...
      var _singleChild = React__default.Children.count(children) === 1;
      //Map requestState to child props via inject function
      if (_singleChild && isFunc(inject)) {
        var _params3 = inject(injection.request);
        var _paramProps3 = isObj(_params3) ? _params3 : { request: _params3 };
        // $FlowFixMe
        return React__default.cloneElement(children, _paramProps3);
      }

      if (inject && _singleChild) {
        // $FlowFixMe
        return React__default.cloneElement(children, injection);
      }
    ...
    if (initialPending) {
      ...
      if (React__default.isValidElement(renderInitial)) {
    ...
    if (inject) {
      ...
      var _singleChild2 = React__default.Children.toArray(children).length === 1;
      //Map requestState to child props via inject function
      if (_singleChild2 && isFunc(inject)) {
        var _params4 = inject(injection.request);
        var _paramProps4 = isObj(_params4) ? _params4 : { request: _params4 };
        // $FlowFixMe
        return React__default.cloneElement(children, _paramProps4);
      }
  ...
  var Request = withRequestSelector(RequestComponent);
   ...
   *```
   */
  var RequestProvider = function (_PureComponent) {
    inherits$2(RequestProvider, _PureComponent);
    ...
    function RequestProvider() {
      ...
      classCallCheck(this, RequestProvider);

      for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
        args[_key] = arguments[_key];
      }

      return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = RequestProvider.__proto__ || Object.getPrototypeOf(RequestProvider)).call.apply(_ref, [this].concat(args))), _this), _this.state = { data: {} }, _this._hasStore = function () {
        return nonEmpty(_this.props.stateProvider);
      }, _this._getRequestStateFromProvider = function () {
        if (_this._hasStore()) {
          return _this.props.stateProvider.getState();
        }
        return {};
      }, _this._getRequestStateFromState = function () {
        return Object.assign({}, _this.state.data);
      }, _this._getRequestState = function () {
        if (_this._hasStore()) {
          return _this._getRequestStateFromProvider();
        }
        return _this._getRequestStateFromState();
      }, _this._putRequestState = function (data) {
        if (_this._hasStore()) {
          return _this.props.stateProvider.putState(data);
        }
        _this.setState({ data: data });
      }, _this._applyStateChange = function (id) {
        return function (transform) {
          if (typeof transform !== "function") return;

          var state = _this._getRequestState();
          var exist = Object.hasOwnProperty.call(state, id);
          var request = exist ? state[id] : initialRequest;
          var req = resetRequestFlags(Object.assign({}, request));
          req.id = id;
          var tdata = transform(req);
          var data = Object.assign({}, state, defineProperty({}, id, tdata));
          _this._putRequestState(data);
        };
      }, _this._requestSuccessful = function (id, message) {
        if (_this._hasStore()) {
          return _this.props.stateProvider.updateRequest({ id: id, message: message, status: SUCCESS });
        }
        var updateSuccess = function updateSuccess(r) {
          var req = r;
          req.success = true;
          req.successCount += 1;
        ...
        _this._applyStateChange(id)(updateSuccess);
      }, _this._requestFailed = function (id, message) {
        if (_this._hasStore()) {
          return _this.props.stateProvider.updateRequest({ id: id, message: message, status: FAILED });
        }
        var updateFailed = function updateFailed(r) {
          var req = r;
          req.failed = true;
          req.failureCount += 1;
        ...
        _this._applyStateChange(id)(updateFailed);
      }, _this._requestPending = function (id, message) {
        if (_this._hasStore()) {
          return _this.props.stateProvider.updateRequest({ id: id, message: message, status: PENDING });
        }
        var updatePending = function updatePending(r) {
          var req = r;
          req.pending = true;
          req.clean = true;
        ...
        _this._applyStateChange(id)(updatePending);
      }, _this._requestClean = function (id) {
        if (_this._hasStore()) {
          return _this.props.stateProvider.updateRequest({ id: id, status: CLEAN });
        }
        var makeClean = function makeClean(r) {
          r.clean = true;
          return r;
        };
        _this._applyStateChange(id)(makeClean);
      }, _this._requestDirty = function (id) {
        if (_this._hasStore()) {
          return _this.props.stateProvider.updateRequest({ id: id, status: DIRTY });
        }
        var makeClean = function makeClean(r) {
          r.clean = false;
          return r;
        ...
        _this._applyStateChange(id)(makeClean);
      }, _this._removeRequest = function (id) {
        if (_this._hasStore()) {
          return _this.props.stateProvider.updateRequest({ id: id, status: REMOVE });
        }
        var state = _this._getRequestState();
        ...
        delete data[id];
        _this._putRequestState(data);
      }, _this._requestActions = function () {
        return {
          success: _this._requestSuccessful,
          failed: _this._requestFailed,
          pending: _this._requestPending,
          remove: _this._removeRequest,
          clean: _this._requestClean,
          dirty: _this._requestDirty
        };
      }, _temp), possibleConstructorReturn(_this, _ret);
    }

    createClass(RequestProvider, [{
      key: "componentDidMount",
      ...
      value: function componentDidMount() {
        ...
        var stateProvider = this.props.stateProvider;

        if (this._hasStore() && isFunc(stateProvider.observe)) {
          stateProvider.observe(function (shouldUpdate) {
            if (shouldUpdate) {
              _this2.forceUpdate();
            }
          });
      ...
      value: function render() {
        ...
        var data = this._getRequestState();
        var actions = this._requestActions();

        var context = { data: data, actions: actions };
  ...
  function withRequest(options) {
    return function withRequestHOC(WrappedComponent) {
      return function (_React$Component) {
        inherits$2(_class2, _React$Component);
        ...
        function _class2() {
          ...
          return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = _class2.__proto__ || Object.getPrototypeOf(_class2)).call.apply(_ref, [this].concat(args))), _this), _this._getIds = function () {
            var id = [];

            if (options) {
              //options.id takes precedence over props.id
              if (isFunc(options.id)) {
                var _id = options.id(_this.props);
                if (_id) {
                  var __id = Array.isArray(_id) ? _id : [_id];
                  id = id.concat(__id);
                }
              } else if (options.id) {
                var _id2 = Array.isArray(options.id) ? options.id : [options.id];
                id = id.concat(_id2);
              }

              //combine props.id if mergeIdSources set true or options.id is empty
              if ((options.mergeIdSources || id.length === 0) && _this.props.id) {
                var _id3 = Array.isArray(_this.props.id) ? _this.props.id : [_this.props.id];
                id = id.concat(_id3);
              }
            }

            return id;
          }, _this._getRequest = function (requestObj) {
            var ids = _this._getIds();
            ...
            if (isEmptyObj(requestObj)) {
              ...
              return ids.reduce(function (acc, id) {
                var iniReq = Object.assign({}, initialRequest, { id: id });
                return Object.assign({}, acc, defineProperty({}, id, iniReq));
              }, {});
            }

            //extract all requestStates by ids
            var requests = arrayValuesOfKeys(requestObj, ids, function (k, o) {
            ...
            if (ids.length === 1) {
              var _req = requests.find(function (r) {
                return r.id === ids[0];
              });
              if (nonEmpty(_req)) {
                return _req;
            ...
            return requests.reduce(function (acc, req) {
              acc[req.id] = req;
              return acc;
            }, {});
          }, _this.renderComponent = function (state) {
            var data = _this._getRequest(state.data);
            var request = Object.assign({}, { data: data, actions: state.actions });

            return React__default.createElement(WrappedComponent, _extends({}, _this.props, { request: request }));
          }, _temp), possibleConstructorReturn(_this, _ret);
        }
  ...
  exports.Provider = RequestProvider;
  exports.withRequest = withRequest;

  Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=questrar.js.map
